...
namespace eka2l1::ios {
    ...
    int launcher::prepare_system_ui_services() {
        ...
        // Phone Mode V9 Codeseg / Nokia C6-00 RM-612:
        ...
        for (const auto &svc : rm612_services) {
            if (process_running(svc.process_name)) {
                LOG_INFO(FRONTEND_CMDLINE,
                    "EKA2L1 Phone Mode V9 Codeseg RM-612: {} already running",
                    svc.process_name);
                ++ready_count;
                continue;
            }
            ...
            if (!proc) {
                LOG_WARN(FRONTEND_CMDLINE,
                    "EKA2L1 Phone Mode V9 Codeseg RM-612: raw spawn failed for {}",
                    svc.process_name);
            ...
            LOG_INFO(FRONTEND_CMDLINE,
                "EKA2L1 Phone Mode V9 Codeseg RM-612: starting raw system process {}",
                svc.process_name);
        ...
        if (alserv && !raw_failed_uids.empty()) {
            for (std::uint32_t uid : raw_failed_uids) {
                ...
                LOG_INFO(FRONTEND_CMDLINE,
                    "EKA2L1 Phone Mode V9 Codeseg RM-612: AppArc fallback for 0x{:08X}", uid);
        ...
        LOG_INFO(FRONTEND_CMDLINE,
            "EKA2L1 Phone Mode V9 Codeseg RM-612: {} UI service(s) running/started",
            ready_count);
