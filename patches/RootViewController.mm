...
- (void)setupPhoneDebugLog {
    ...
    NSURL *url = [docs URLByAppendingPathComponent:@"eka2l1-phone-v11-debug.log"];
    ...
    if (path) {
        ...
        fprintf(stderr, "=== EKA2L1 Phone Mode V11 WINDOW ===\n");
...
- (void)sharePhoneDebugLog {
    ...
    // Phone Mode V11 WINDOW: share the wrapper capture together with the two
...
// ---- Symbian Phone Mode ---------------------------------------------------
// Phone Mode V11 WINDOW: delayed UIKit transition + Avkon UI-service bootstrap + dedicated system-ui lifecycle.
...
- (void)launchPhoneUid:(std::uint32_t)uid name:(NSString *)name {
    ...
    NSLog(@"EKA2L1 Phone Mode V11 WINDOW: bootstrapping RM-612 Starter UI services before command_run %@ (0x%08X)", name, uid);
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"EKA2L1 Phone Mode V11 WINDOW: %d UI service registration(s) prepared", prepared);
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(warmup * NSEC_PER_SEC)),
                ...
                NSLog(@"EKA2L1 Phone Mode V11 WINDOW: command_run %@ (0x%08X)", launchName, uid);
...
- (void)onAppExited {
    if (self.phoneMode) {
        NSLog(@"EKA2L1 Phone Mode V11 WINDOW: ignoring normal game-exit callback for system UI");
        return;
    }
