...
- (void)setupPhoneDebugLog {
    ...
    NSURL *url = [docs URLByAppendingPathComponent:@"eka2l1-phone-v12-debug.log"];
    ...
    if (path) {
        ...
        fprintf(stderr, "=== EKA2L1 Phone Mode V12 RAMLIFE ===\n");
...
- (void)sharePhoneDebugLog {
    ...
    // Phone Mode V12 RAMLIFE: share the wrapper capture together with the two
    // rotating core logs when they exist. This keeps the exact Home/Menu test
    // session available without requiring the user to reopen EKA2L1 first.
    NSMutableArray *items = [NSMutableArray arrayWithObject:url];
    NSURL *docs = [url URLByDeletingLastPathComponent];
    for (NSString *name in @[@"EKA2L1.log", @"EKA2L1_TakeThis.log"]) {
        NSURL *coreURL = [docs URLByAppendingPathComponent:name];
        if ([[NSFileManager defaultManager] fileExistsAtPath:coreURL.path]) {
            [items addObject:coreURL];
        }
    }
...
// ---- Symbian Phone Mode ---------------------------------------------------
// Phone Mode V12 RAMLIFE: delayed UIKit transition + Avkon UI-service bootstrap + dedicated system-ui lifecycle.
...
- (void)launchPhoneUid:(std::uint32_t)uid name:(NSString *)name {
    ...
    NSLog(@"EKA2L1 Phone Mode V12 RAMLIFE: bootstrapping RM-612 Starter UI services before command_run %@ (0x%08X)", name, uid);
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"EKA2L1 Phone Mode V12 RAMLIFE: %d UI service registration(s) prepared", prepared);
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(warmup * NSEC_PER_SEC)),
                ...
                NSLog(@"EKA2L1 Phone Mode V12 RAMLIFE: command_run %@ (0x%08X)", launchName, uid);
...
- (void)onAppExited {
    if (self.phoneMode) {
        NSLog(@"EKA2L1 Phone Mode V12 RAMLIFE: ignoring normal game-exit callback for system UI");
        return;
    }
